    ...
    def sampling_generator(self, num_samples=1000, batch_size=8, aug=True):
        for i in range(num_samples // batch_size):
            ...
            for i in range(batch_size):
                sample_i, sample_i_e = self.sample()
                sample.append(sample_i)
                samplee.append(sample_i_e)
            sample = np.concatenate(sample, axis=0)
            samplee = np.concatenate(samplee, axis=0)
            if aug is True:
                ...
                if np.random.uniform() > 0.5:  # y-z mirroring
                    sample[..., 1] = - sample[..., 1]
            yield sample
    def sampling_generator_energy(self, num_samples=1000, batch_size=8, aug=True):
        for i in range(num_samples // batch_size):
            sample = []
            samplee = []
            for i in range(batch_size):
