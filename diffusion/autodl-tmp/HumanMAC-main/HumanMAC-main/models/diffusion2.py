...
class Diffusion2:
    ...
    def noise_motion(self, x, t,sigma,mu):
        sqrt_alpha_hat = torch.sqrt(self.alpha_hat[t])[:, None, None]
        sqrt_one_minus_alpha_hat = torch.sqrt(1 - self.alpha_hat[t])[:, None, None]
        Ɛ = torch.randn_like(x) * sigma + mu
        return sqrt_alpha_hat * x + sqrt_one_minus_alpha_hat * Ɛ, Ɛ

    def sample_timesteps(self, n):
        return torch.randint(low=1, high=self.noise_steps, size=(n,))

    def inpaint_complete(self, step, x, prev_t, traj_dct, mode_dict,sigma,mu):
        """
        ...
        Returns:
            completed sample
        """
        x_prev_t_known, _ = self.noise_motion(traj_dct, prev_t,sigma,mu)  # add noise in DCT domain
        x_prev_t_known = torch.matmul(self.idct[:, :(self.n_pre-1)],
                                      x_prev_t_known[:, :(self.n_pre-1)])

        #x_prev_t_unknown = torch.matmul(self.idct[:, :(self.n_pre-1)],
                                        #x[:, :(self.n_pre-1)])
        x = x_prev_t_known
    ...
    def sample_ddim_progressive(self, model, traj_dct, traj_dct_mod, mode_dict, sigma,mu,features):
        """
        ...
        x_ = torch.randn((sample_num, self.motion_size[0], self.motion_size[1])).to(self.device)
        ...
        with torch.no_grad():
            for i in reversed(range(0, self.ddim_timesteps)):
                ...
                predicted_noise = model(x_, t, features, mod=traj_dct_mod)
                predicted_x0 = (x_ - torch.sqrt((1. - alpha_hat)) * predicted_noise) / torch.sqrt(alpha_hat)
