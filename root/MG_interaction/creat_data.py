...
data = np.load('/root/autodl-tmp/rmd17/npz_data/rmd17_aspirin.npz')
print(data.files)
pos = np.load('/root/autodl-tmp/interpolated_trajectory.npy')
numbers = data["nuclear_charges"]
atoms_list = []
property_list = []
for positions, energies, forces in zip(data["coords"][:20000], data["energies"][:20000], data["forces"][:20000]):
...
for prop_dict in property_list:
    for key in prop_dict:
        ...
        if isinstance(prop_dict[key], (float, int)):
            ...
            prop_dict[key] = np.array(prop_dict[key], dtype=np.float32)
db_path = './new_dataset_asp_20000.db'
